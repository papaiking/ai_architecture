# Sources Index

Danh sách tất cả sources trong thư mục này, được phân loại theo title (A-Z).

## A

- [Aider](./aider.md) - Aider (AI pair programming, 43k stars)
- [AionUi](./aionui.md) - AionUi (22.3k stars)
- [AnythingLLM](./anything_llm.md) - AnythingLLM (58.7k stars)
- [Agentic AI: The Big Picture](./agentic_ai_big_picture.md) - Agentic AI: The Big Picture
- [AI Engineering from Scratch](./ai_engineering_from_scratch.md) - AI Engineering from Scratch (260+ lessons)
- [AnythingLLM](./anything_llm.md) - AnythingLLM (58.7k stars)
- [Archon](./archon.md) - Archon (workflow engine for coding agent)
- [AutoResearchClaw](./autoresearchclaw.md) - AutoResearchClaw (research pipeline)
- [Autoskills](./autoskills.md) - Autoskills (CLI tool for AI skills)
- [Awesome LLM Apps](./awesome_llm_apps.md) - Awesome LLM Apps (100+ templates)

## C

- [Claude-Mem](./claude_mem.md) - Claude-Mem (persistent memory)
- [Cline](./cline.md) - Cline (autonomous coding agent, 60k stars)
- [code-review-graph](./code_review_graph.md) - code-review-graph (knowledge graph, 13k stars)
- [Crucix](./crucix.md) - Crucix (personal intelligence agent)

## D

- [Deep Agents](./deep_agents.md) - Deep Agents (agent framework)
- [Dots.ocr](./dots_ocr.md) - dots.ocr (document layout parsing)
- [Feynman](./feynman.md) - Feynman (AI research agent)

## H

- [Hermes Agent](./hermes_agent.md) - Hermes Agent (personal AI)

## L

- [LLM Architecture Gallery](./llm_architecture_gallery.md) - LLM Architecture Gallery (70+ models)
- [Langfuse](./langfuse.md) - Langfuse (LLM observability platform)
- [LFM2.5-Audio-1.5B](./lfm2_5_audio.md) - LFM2.5-Audio-1.5B (audio foundation model)
- [LLM Architecture Gallery](./llm_architecture_gallery.md) - LLM Architecture Gallery (70+ models)

## M

- [MedGemma](./medgemma.md) - MedGemma (medical AI LLM)
- [Multica](./multica.md) - Multica (agent platform)

## N

- [Free NVIDIA APIs](./nvidia_free_apis.md) - Free hosted APIs for 70+ open source models

## O

- [oLLM](./ollm.md) - oLLM (large-context LLM inference)
- [Onyx](./onyx.md) - Onyx (open source AI platform, 27.4k stars)
- [Open CoDesign](./open_codesign.md) - Open CoDesign (AI design tool)
- [OpenDataLoader PDF](./opendataloader_pdf.md) - OpenDataLoader PDF (PDF extraction)
- [OpenTelemetry](./opentelemetry.md) - OpenTelemetry (industry-standard observability)

## P

- [Poolside Laguna](./poolside_laguna.md) - Poolside Laguna (open source coding model)
- [Proxy-Pointer RAG](./proxy_pointer_rag.md) - Proxy-Pointer RAG (vectorless accuracy)

## Q

- [QwenPaw](./qwenpaw.md) - QwenPaw (personal AI assistant)

## S

- [Scrapling](./scrapling.md) - Scrapling (web scraping)
- [Segmentation with Internet-Augmented Multimodal LLMs](./segmentation_internet_rag.md) - Segmentation with Internet-Augmented LLMs
- [SkillClaw](./skillclaw.md) - SkillClaw (skill evolution system)
- [Strix](./strix.md) - Strix (security testing)

## T

- [The Anatomy of an Agent Harness](./agent_harness.md) - The Anatomy of an Agent Harness
- [TurboQuant: 3-bit KV Cache with Near 0 Accuracy Loss](./turboquant.md) - TurboQuant (KV cache quantization)
- [72 Techniques to Optimize LLMs in Production](./llm_optimization_72_techniques.md) - 72 Techniques to Optimize LLMs